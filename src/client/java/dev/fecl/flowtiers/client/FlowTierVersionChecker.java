...
public final class FlowTierVersionChecker {
	...
	private static void notifyIfOutdated(MinecraftClient client, String latestVersion) {
		if (client.player == null || compareVersions(latestVersion, currentVersion()) <= 0) return;

		MutableText message = Text.literal("[FlowTiers] ")
				.formatted(Formatting.AQUA)
				.append(Text.literal("A new version is available: ").formatted(Formatting.GRAY))
				.append(Text.literal(latestVersion).formatted(Formatting.GREEN))
				.append(Text.literal(" (you have " + currentVersion() + "). ").formatted(Formatting.GRAY))
				.append(Text.literal(DOWNLOAD_URI.toString()).formatted(Formatting.YELLOW));
