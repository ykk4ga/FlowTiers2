...
public final class FlowTierPlayerStatsScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
		...
		context.text(font, "Tier", panelLeft + 150, top - 14, 0xFFB5C7E8, true);
		context.text(font, "ELO", panelLeft + 254, top - 14, 0xFFB5C7E8, true);
		context.text(font, "Pos", panelLeft + 316, top - 14, 0xFFB5C7E8, true);
		context.text(font, "W/L", panelLeft + 368, top - 14, 0xFFB5C7E8, true);
		context.text(font, "Streak", panelLeft + 426, top - 14, 0xFFB5C7E8, true);
		...
		for (int i = 0; i < ladders.size(); i++) {
			...
			context.text(font, ladder.tierLabel(), panelLeft + 150, y + 3, tierColor(ladder.tierLabel(), ladder.position()), true);
			context.text(font, ladder.totalRating() + " ELO", panelLeft + 254, y + 3, eloColor(ladder.totalRating()), true);
			context.text(font, ladder.hasPosition() ? "#" + ladder.position() : "-", panelLeft + 316, y + 3, ladder.hasPosition() ? 0xFFFFD700 : 0xFF7C8BA1, true);
			context.text(font, ladder.wins() + "/" + ladder.losses(), panelLeft + 368, y + 3, winLossColor(ladder.wins(), ladder.losses()), true);
			context.text(font, streak(ladder.currentStreak()), panelLeft + 426, y + 3, streakColor(ladder.currentStreak()), true);
			y += rowHeight;
		}

		playerStats.bestLadder().ifPresent(best -> context.text(font,
				"Highest: " + FlowTierFormatter.displayName(best.ladder()) + " " + best.tierLabel(),
				panelLeft + 12, height - 46, 0xFFFFD700, true));
	}
