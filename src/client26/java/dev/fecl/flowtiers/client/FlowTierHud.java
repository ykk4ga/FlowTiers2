...
public final class FlowTierHud {
	...
	private static void draw(GuiGraphicsExtractor context, Minecraft client, FlowTierStats stats) {
		...
		int tx = PADDING;
		int ty = PADDING;
		context.text(font, header, tx, ty, FLOW_BLUE, true);
		if (icon != null) {
			context.text(font, icon, tx + font.width(header) + font.width("  "), ty, WHITE, true);
		}
		ty += LINE_HEIGHT;

		context.text(font, tierLine, tx, ty, tierColor, true);
		ty += LINE_HEIGHT;
		...
		if (recordLine != null) {
			context.text(font, recordLine, tx, ty, GRAY, true);
		}
